...
public class ImageController {
	...
	public String insertImageProc(ImageVO vo, HttpSession session,
		...
		if(!file.isEmpty()) {
			String uploadDir = session.getServletContext().getRealPath("/upload/imageboard_main/");
			String uploadThumb = session.getServletContext().getRealPath("/upload/imageboard_thumb/");
	...
	public String deleteImage(ImageVO vo, HttpSession session) {
		...
		File file1 = new File(session.getServletContext().getRealPath("/upload/imageboard_main/") + dbVO.getUploadFileName());
		File file2 = new File(session.getServletContext().getRealPath("/upload/imageboard_thumb/") + dbVO.getUploadFileName());
